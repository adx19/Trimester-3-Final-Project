  ...
  try {
    ...
    for (let i = 0; i < maxLookbackDays; i++) {
      ...
      const leagueMatches = events.filter((event) => {
        ...
        const isPast = event.startTimestamp * 1000 < now;
        return isFinished && isPast && tournamentId === leagueId;
      });
  ...
  try {
    ...
    for (let i = 0; i < maxForwardDays; i++) {
      ...
      const upcomingMatches = events.filter((event) => {
        const tournamentId = event.tournament?.uniqueTournament?.id;
        const isLater = event.startTimestamp * 1000 > now;
        return isLater && tournamentId === leagueId;
      });
