  ...
  try {
    ...
    for (let i = 0; i < maxLookbackDays; i++) {
      ...
      const leagueMatches = events.filter((event) => {
        ...
        const categorySlug = event.tournament?.category?.slug?.toLowerCase() || "";
      ...
      if (leagueMatches.length > 0) {
        return leagueMatches.map((event) => ({
          ...
          venue: event.venue?.stadium?.name || event.venue?.name || "Unknown Venue",
          date: dateStr,
  ...
  try {
    ...
    return filteredEvents.map((event) => {
      ...
      return {
        ...
        time: new Date() - event.startTimestamp
          ? new Date(event.startTimestamp * 1000).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })
